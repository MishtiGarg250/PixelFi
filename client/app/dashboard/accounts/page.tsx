    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-neutral-400 mt-1">
          Select a portfolio to view and manage its connected accounts.
        </p>
      </div>
      ...
      {!portfolios.isLoading && portfolios.data?.length === 0 && (
        <div className="flex flex-col items-center justify-center py-24 rounded-2xl border border-dashed border-white/8 bg-neutral-950/20 text-center">
          <Wallet size={28} className="text-neutral-600 mb-3" />
          <p className="text-sm text-neutral-500 mb-4">No portfolios found. Create a portfolio first to manage accounts.</p>
          <button
      ...
      {portfolios.data && portfolios.data.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {portfolios.data.map((p) => (
            <button
              key={p.id}
              onClick={() => router.push(`/dashboard/portfolios/${p.id}?tab=accounts`)}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-neutral-500 mt-0.5">
                    {p._count?.accounts ?? 0} account{(p._count?.accounts ?? 0) !== 1 ? "s" : ""}
                  </p>
                </div>
              </div>
